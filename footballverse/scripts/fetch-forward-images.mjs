// Run: node scripts/fetch-forward-images.mjs
const forwards = [
    // Current STs
...
async function main() {
    ...
    for (let i = 0; i < forwards.length; i++) {
        ...
        console.log(`[${String(i + 1).padStart(2, '0')}/${forwards.length}] ${symbol} ${forwards[i].query.padEnd(28)} → apiName: ${result.apiName || 'N/A'} | ${result.thumb ? result.thumb.split('/thumb/')[1] : result.status}`);
        if (i < forwards.length - 1) await delay(2500);
    }

    console.log('\n\n// ===== IMAGE MAP =====');
    console.log('const IMAGE_MAP = ' + JSON.stringify(
    ...
    if (missing.length) {
        console.log(`\n// ===== MISSING / NO IMAGE (${missing.length}) =====`);
        missing.forEach(r => console.log(`// REMOVE: { id: '${r.id}', query: '${r.query}', status: '${r.status}' }`));
    }
    console.log('\nDone! Copy the IMAGE_MAP above and update data.js');
}
main();
